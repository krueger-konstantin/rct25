---
title: "Empirical Assignment Step 5: Firm Comparison Table"
...
```{r step5-table, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
library(tidyverse)
library(stringdist)
library(lubridate)
library(broom)
...
Notes: This Orbis dataset is provided by WRDS. The sample covers the period of 2021. Firms with total assets of zero were excluded. Total assets are reported in euros. The equity ratio is defined as book value of equity divided by total assets and expressed as a percentage. Mean comparisons between firms in postal code 10435 and the overall Berlin sample are based on t-tests. Corresponding p-values indicate statistical significance